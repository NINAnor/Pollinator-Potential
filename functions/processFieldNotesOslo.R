...
#' 
#' 
processFieldNotesOslo <- function(focalEndpoint, dataFolder, datasetName, regionGeometry, focalTaxon) {
  ...
  #options(timeout=100)
  download.file(focalEndpoint, paste0(dataFolder,"/", datasetName ,".zip"), mode = "wb")
  unzip(paste0(dataFolder,"/", datasetName ,".zip"), exdir = paste0(dataFolder,"/",  datasetName))
  
  # Load in occurrence data
  occurrence <- read.delim(paste0(dataFolder,"/", datasetName ,"/occurrence.txt"))
