...
processNationalInsectMonitoringOld <- function(focalData, endpoint, tempFolderName, regionGeometry) {
  ...
  # First, get a list of all taxon sampled samples and create a legend for them
  ...
  #colnames(ourDataset)[5] <- "order"
  
  # Now get abundances for those taxon that have them
  ourDatasetAbundance <- merge(ourDataset, occurrencesWithEvent[,c("scientificName", "organismQuantity", "parentEventID",
                                                                   "locationID", "order")], all.x = TRUE, 
                               by = c("parentEventID", "locationID", "scientificName"))%>%
    dplyr::filter(!is.na(order))%>%
    dplyr::group_by(locationID, order, parentEventID, year)%>%
    summarise_at(.vars = "organismQuantity", .funs = "sum", na.rm = TRUE)%>%
      #distinct() %>%
    dplyr::ungroup()
  ...
  # Cut down set
  newDataset <- newDataset[c("order", "organismQuantity", "year")] %>%
    rename(individualCount = organismQuantity)
  
  # Crop to relevant region
  #st_crs(newDataset) <- "+proj=longlat +ellps=WGS84"
  newDataset <- st_transform(newDataset, crs = st_crs(regionGeometry))
  newDataset <- st_intersection(newDataset, regionGeometry)
  
  newDataset$dataType <- "Counts"
  
  taxaLegend <- distinct(st_drop_geometry(focalData[,c("taxa", "order", "taxonKeyProject")]))
  
  newDataset$taxa <- taxaLegend$taxa[match(newDataset$order, taxaLegend$order)]
  newDataset$taxonKeyProject <- taxaLegend$taxonKeyProject[match(newDataset$order, taxaLegend$order)]
  return(newDataset)
