...
estimateTaxonDiversityAlt <- function(pollinatorSpeciesNames = NULL, #Names of the pollinator species (should be the simplescientific name)
  ...
  zones <- unique(dat$zone)
    ...
    for(j in 1:length(sp)){
      ...
      diversity <-  lapply(plantSpecies, function(plantSp){
        divs <-  round(results$intensity * speciesData[[plantSp]] * as.numeric(c(interractionProb[interractionProb$species %in% plantSp, genera[j]])), digits = 1)
      ...
      PredictionData <- PredictionData%>%
        dplyr::mutate(averageIntensity = rowMeans(pick(all_of(plantSpecies))))%>%
        dplyr::select(all_of(c("averageIntensity","geometry")))#%>%
        #dplyr::mutate(averageIntensity = round(averageIntensity, 1)*10)
      
      PredictionData$averageIntensity <- round(PredictionData$averageIntensity, digits = 1)*10
    ...
    print("Create path for results")
    path <- paste0(dataFolder, "/modelOutputs/diversity/", taxa)
    ...
    st_write(sf_combined, file.path(path, paste0("predictions.shp")))
    
    #saveRDS(sf_combined, file.path(path, paste0("predictions.rds")))
    #rm("spPred")
