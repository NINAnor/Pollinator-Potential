...
#' 
#' 
processFieldNotes <- function(focalEndpoint, tempFolderName, datasetName, regionGeometry, taxa) {
  ...
  if (!("eventID" %in% colnames(occurrence))) {
    if(!("eventData" %in% colnames(occurrence))){
      occurrence$eventDate <- paste(occurrence$year, occurrence$month, occurrence$day, sep = "-")
    } else if(length(unique(occurrence$eventDate)) == 1 &  is.na(unique(occurrence$eventDate))) {
  ...
  eventLocationsSF <- st_intersection(eventLocationsSF, 
                                      st_transform(regionGeometry, crs(eventLocationsSF)))
