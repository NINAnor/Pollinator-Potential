...
corineReclassification <- function(zip_path = NA, output_path = NA) {
  ...
  if(!file.exists(paste0(dataFolder,"/corineReclassification.csv"))) {
    warning("No CORINE reclassification system has been provided. Using all 52 potential categories.")
  } else {
    corineReclassification <- read.csv(paste0(dataFolder,"/corineReclassification.csv"), header = TRUE)
    ...
    #levels(corine) <- unique(corineReclassification$newCategory)
  }
